...
----
arr.push(i1, i2, ...i)
// arr의 끝에 하나 이상의 i 추가 (원본 arr 변경), arr의 새로운 길이를 반환. (원본 변경)
----
...
// start 부터 n개 (옵션 - 기본: end 까지) item 삭제 후 그 자리에 하나 이상의 i (옵션 - 기본: 없음) 추가 (원본 arr 변경), 제거한 item들로 구성된 arr 반환
----
...
// start (옵션 - 기본: 0) 에서 end (옵션 - 기본: arr.length) 까지 item을 i로 채움 (원본 arr 변경), 변경된 arr 반환
----
...
----
arr.concat(h1, h2, ...h) 
// arr에 하나 이상의 h[arr, item] 를 합쳐 새로운 arr 반환
----
...
----
arr.indexOf(i, n) 
// arr의 item 중 i의 존재 여부 확인, n 번째 (옵션 - 기본: 1) index를 반환, 없으면 -1 반환
----
...
[source,js]
----
// arr.splice(start, n, i1, i2, ...i)  - 원본 arr 변경
arr.splice(target, 1)
----
[source,js]
----
...
[source,js]
----
// arr.unshift(i) / arr.push(i1, i2, ...i) - 원본 arr 변경
arr.unshift(i)
arr.push(i)
----
[source,js]
