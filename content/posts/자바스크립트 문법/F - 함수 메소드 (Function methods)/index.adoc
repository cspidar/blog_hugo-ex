---
title: "자바스크립트 문법 - G - 함수 주요 메소드 (Function methods essentials)"
...
=== 특징
this 키워드는 함수 내에서 현재 실행 중인 함수를 참조하지 않으므로 이름으로 Function 객체를 직접 참조해야 함 (bind() 메서드 활용 이유)
...
//p_this와 하나 이상의 p (arr 형태)로 func 호출합니다.	
// call() 와 유사, call() 은 함수에 전달될 인수 리스트를 받는데 비해, apply() 는 인수들의 단일 배열을 받는다	
----
