---
title: "자바스크립트 알고리즘 - 코드 - 우선 순위 큐 & 다익스트라"
date: "2021-06-24"
categories: 
  - 자바스크립트 알고리즘
tags:
...
* 출처: https://jun-choi-4928.medium.com/javascript로-heap-priority-queue-구현하기-8bc13bf095d9

.활용 형태
[source,JS]
----
const pq = new PriorityQueue(); // 우선 순위 큐 생성
pq.enqueue(key, value) // value 추가
pq.dequeue() // value 삭제
pq.isEmpty() // value 존재 여부 확인
----
...
* 위의 '우선 순위 큐' 사용
* 출발 노드에서 각 노드까지의 최단 시간을 distance 배열에 담아 반환
...
. '출발 노드' enqueue
. dequeue
. '출발 노드' 에서 '현 위치' 를 경유한 각각 '다음 노드' 까지의 거리 (출발 노드 -> 현 위치 -> 다음 노드) 가 기존 거리 보다 작으면, 거리를 distance 배열에 저장하고 '다음 노드' 를 enqueue
. 2 - 3 반복, dequeue 할 노드가 존재하지 않으면 종료
